...
> **Obsidian is optional.** This skill writes plain, well-structured markdown
> files (with Obsidian-flavored conventions: `[[wiki-links]]`, `[!callouts]`,
> YAML frontmatter) to a notes folder. Nothing here calls the Obsidian app or
> requires it to be installed. If you have Obsidian, point it at that folder
> as a vault and you get backlinks, graph view, and rendered callouts for
> free. If you don't, the files are still perfectly readable/editable markdown
> with any editor, the wiki-links and callouts just render as plain text
> instead of being clickable/styled. See the companion `obsidian-vault`
> skill for the folder structure and formatting conventions used here.
...
flowchart TD
    ...
    Save[Save to notes folder] --> End([Done])
...
2. Show yesterday's "Tomorrow's focus", "Open questions"
3. Ask:
...
**Running the script:**
```bash
uv run scripts/collect_day_data.py [--date YYYY-MM-DD] [--projects-dir ~/Projects] [--skip-sessions]
```

`--projects-dir` defaults to `~/Projects`; pass your own if your repos live elsewhere.

## Markdown Conventions

Clearly differentiate personal voice from LLM context using Obsidian-compatible markdown (renders fine as plain markdown even without Obsidian):
